"""Import fruit metadata into the FoodVision knowledge base."""
...
def extract_records(source_data: Any) -> list[dict[str, Any]]:
    """Extract fruit records from a loaded CSV or JSON source."""
    ...
    raise ValueError("Fruit source must be a list or contain a list of records")
...
def map_fruit_to_food(row: dict[str, str]) -> dict[str, Any]:
    """Convert a normalized fruit row into a FoodVision food object."""
...
def validate_fruit_food(food: dict[str, Any]) -> list[str]:
    """Validate a fruit food entry using the existing FoodVision validator."""
    ...
    if food.get("usda") is not None:
        errors.append("usda must be null for fruit imports")

    return errors


def load_fruit_collection(collection_path: Path) -> tuple[Any, list[dict[str, Any]]]:
    """Load fruits.json and return the original payload plus its foods list."""
    ...
    if not isinstance(collection, dict):
        raise ValueError(f"Invalid fruits collection format: {collection_path}")
    ...
    if foods is None:
        ...
        raise ValueError(f"fruits.json foods must be a list: {collection_path}")

    return collection, foods


def existing_fruit_keys(foods: list[dict[str, Any]]) -> tuple[set[str], set[str]]:
    """Return existing fruit ids and normalized display names."""
...
def print_fruit_summary(
    records_read: int,
    imported: int,
    duplicates: int,
    invalid: int,
) -> None:
    """Print a concise fruit import summary."""

    skipped = duplicates + invalid

    print("=" * 38)
    print("Fruit Import Summary")
    print("=" * 38)
    print()
...
def import_fruits(
    ...
    """Import fruit metadata into knowledge_base/fruits.json."""
    ...
    collection_path = knowledge_base_dir / "fruits.json"
    collection, foods = load_fruit_collection(collection_path)
    existing_ids, existing_display_names = existing_fruit_keys(foods)

    imported = 0

    for row in unique_rows:
        food = map_fruit_to_food(row)
        ...
        errors = validate_fruit_food(food)
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the fruit importer."""

    project_root = Path(__file__).resolve().parents[2]

    parser = argparse.ArgumentParser(
        description="Import fruit metadata into FoodVision."
    )
    parser.add_argument(
        "source_path",
        nargs="?",
        type=Path,
        default=project_root / "data" / "imports" / "fruits.csv",
        help="Path to a CSV or JSON fruit dataset.",
    )
    parser.add_argument(
        "--knowledge-base-dir",
        type=Path,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate and summarize without writing fruits.json.",
    )
